...
- Composer 统计栏使用戒指标签作为左端徽记、圣树作为低透明度水印；统计文字保持宿主顺序和语义，不注入或改写费用数据。
- 已有数据的轨迹时间轴只展示紧凑时序之剑标识，空轨迹保留较大的藏品构图；宿主未提供可靠运行状态，因此不伪造执行进度动画。
- 右侧详情栏保持同一“藏品柜”位置：聊天视图展示童年唱片，轨迹视图展示时序之剑；选中真实工具详情后立即隐藏藏品。
- 烤肉吊牌只在空会话 `hero` 阶段、且人物舞台宽度足够时出现，进入工作阶段或紧凑布局后退出。
